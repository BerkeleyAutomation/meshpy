...
class UrdfWriter(object):
    ...
    def write(self, mesh):
        ...
        if not os.path.exists(out_dir):
            os.mkdir(out_dir)
        vhacd_out_filename = os.path.join(out_dir, '%s_vhacd.obj' %(self.name_))
        log_filename = os.path.join(out_dir, 'vhacd_log.txt')
        
        # save mesh to directory
        obj_filename = os.path.join(out_dir, '%s.obj' %(self.name_))
